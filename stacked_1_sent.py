...
df = pd.read_csv(open('../radical_sent/data/corpus.csv', 'r', encoding='UTF-8'))
labels = df['label']
...
for slices in range(0, 5):
    with open('./k-folds-sent/test_{}.plk'.format(slices), 'rb') as f:
        test = pickle.load(f)
...
for idx, model_name in enumerate(models):
    ...
    for s in range(0, 5):
        with open('./semantic-only/{}/model/sent_train_{}_{}_{}.plk'.
...
for idx, model_name in enumerate(models):
    avg_arr = 0
    for s in range(0, 5):
        with open('./semantic-only/{}/model/sent_test_{}_{}_{}.plk'.
                          format(model_name, model_name, s, optimizers[idx]), 'rb') as f:
            arr = pickle.load(f)
            avg_arr += arr/5
